...
{
    public class IncidentPointsSelectionWindow : Window
    {
        ...
        public IncidentPointsSelectionWindow(IncidentDef incidentDef, Action<IncidentDef, float> onPointsSelected)
        {
            this.incidentDef = incidentDef;
            this.onPointsSelected = onPointsSelected;
            pointOptions = DebugActionsUtility.PointsOptions(extended: true).Distinct().OrderBy(p => p).ToList();
            pointOptions.Add(50000f);
            pointOptions.Add(100000f);
        ...
        public override void DoWindowContents(Rect inRect)
        {
            ...
            Widgets.Label(new Rect(inRect.x, inRect.y, inRect.width, 36f), "CheatMenu.Incidents.IncidentPointsWindow.Title".Translate());

            Text.Font = GameFont.Small;
            Widgets.Label(
                new Rect(inRect.x, inRect.y + 28f, inRect.width, 24f),
                "CheatMenu.Incidents.IncidentPointsWindow.Subtitle".Translate(incidentDef?.LabelCap ?? incidentDef?.defName ?? string.Empty));
        ...
        private void DrawPointsList(Rect outRect)
        {
            ...
            for (int i = 0; i < pointOptions.Count; i++)
            {
                ...
                string label = "CheatMenu.Incidents.IncidentPointsWindow.PointsButton".Translate(points.ToString("F0"));
                if (!IncidentDoIncidentCheat.CanFireNowWithPoints(incidentDef, points))
                {
                    label += " [NO]";
                }
